...
from train_velodyne_td3_1 import main_run
from velodyne_env_1 import GazeboEnv
NUM_STATES = 8
BEHAVIOR = 'direction'  # yellow/brown/red/bumping
...
class irlAgent:
    ...
    #     saved_model = 'saved-models_' + self.behavior + '/evaluatedPolicies/' + str(i) + '-164-150-100-50000-' + str(
    #         self.num_frames) + '.h5'  # use the saved model to get the FE
    ...
    def policyListUpdater(self, W, i):  # add the policyFE list and differences
        ...
        tempFE = train_run.IRL_run(W, env)
...
if __name__ == '__main__':
    ...
    randomPolicyFE = [7.74363107e+05, 4.83296402e+04, 6.1289194e+05, 0.39292849e+04, 2.0488831e+05, 0.65611318e-26,
                      6.90207523e+04, 0.16475348, 7.74363107e+05, 4.83296402e+04, 6.1289194e+05, 0.39292849e+04, 2.0488831e+05, 0.65611318e-26,
                      6.90207523e+04, 0.16475348, 7.74363107e+05, 4.83296402e+04, 6.1289194e+05, 0.39292849e+04, 2.0488831e+05, 0.65611318e-26,
                      6.90207523e+04, 0.16475348]
    # ^the random policy feature expectations
    directionPolicyFE = [136544.20625782, 1809.9153963, 26225.1555566, 21314.77115228, 88508.80884812, 128150.57604946,
                         73485.23433685, 0.0, 136544.20625782, 1809.9153963, 26225.1555566, 21314.77115228, 88508.80884812, 128150.57604946,
                         73485.23433685, 0.0, 136544.20625782, 1809.9153963, 26225.1555566, 21314.77115228, 88508.80884812, 128150.57604946,
                         73485.23433685, 0.0]
    # feature expection for the directive behavior
    ...
    # expertPolicyBumpingFE = [1.62424521e+05, 7.41818200e+04, 1.61867705e+05, 6.33978371e+04, 2.27695277e+05,
    #                          7.00162455e-26, 1.38607034e+04, 0.00000000e+00]
    # ^feature expectations for the "nasty bumping" behavior
    epsilon = 0.5
    irlearner = irlAgent(randomPolicyFE, directionPolicyFE, epsilon, NUM_STATES, FRAMES, BEHAVIOR)
    w = irlearner.optimization()
    environment_dim = 20
    robot_dim = 4
    env = GazeboEnv("multi_robot_scenario.launch", environment_dim, w)
    train_run = main_run()
