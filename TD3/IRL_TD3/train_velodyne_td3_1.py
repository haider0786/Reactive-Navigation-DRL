...
class main_run(object):
    def IRL_run(self, w, env):
        eval_cls = eval_net()
        self.env = env
        # Set the parameters for the implementation
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # cuda or cpu
        seed = 0  # Random seed number
        eval_freq = 100  # After how many steps to perform the evaluation
        ...
        environment_dim = 20
        robot_dim = 4
        # self.env = GazeboEnv("multi_robot_scenario.launch", environment_dim, w)
        time.sleep(5)
        ...
        while timestep < max_timesteps:
            ...
            if done:
                ...
                #     evaluations.append(
                #         eval_cls.evaluate(w, self.env, network=network, epoch=epoch, eval_episodes=eval_ep)
                #     )
        ...
        average_reward, featureExpectations = eval_cls.evaluate(w, self.env, network=network, epoch=epoch, eval_episodes=eval_ep)
        temp_av = (average_reward, featureExpectations)
        evaluations.append(temp_av)
        if save_model:
            network.save("%s" % file_name, directory="./models_1")
        np.save("./results_1/%s" % file_name, evaluations)
        # print("evaluations[1]", evaluations[1])
        return featureExpectations

class eval_net(object):
    def evaluate(self, w, env, network, epoch, eval_episodes=1):
        avg_reward = 0.0
        col = 0
        GAMMA = 0.9
        featureExpectations = np.zeros(len(w))
        for _ in range(eval_episodes):
            ...
            while not done and count < 501:
                ...
                featureExpectations += (GAMMA * np.array(state))
                avg_reward += reward
                count += 1
                if reward < -90:
                    col += 1
        ...
        print("..............................................")
        print("featureExpectations", featureExpectations)
        return avg_reward, featureExpectations
