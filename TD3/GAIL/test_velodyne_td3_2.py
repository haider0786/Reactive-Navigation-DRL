...
while True:
    ...
    next_state, reward, done, target, dis_r1r2, dist, length, gx, gy, rx, ry = env.step(a_in)
    ...
    if target:
        ...
        results_save = list([rx, ry]), list([gx, gy]) + list([dis_r1r2]) + list([int(done)]) \
                       + list([length]) + list([time_elap])
        results_app.append(results_save)
        sardns_data = pd.DataFrame(results_app)
        sardns_data.to_csv('/home/haider/DRL-robot-navigation/TD3/GAIL/trained_models/obst_traj/headon' + ".csv")
        start = time.time()
